...
import { faBars, faXmark, faHome, faPlus, faPen, faList, faBell, faTruck } from '@fortawesome/free-solid-svg-icons'
    ...
    return (
        <>
            <div className="flex items-center justify-between px-4 md:px-6 lg:px-12 py-2.5 md:py-3 border-b border-slate-200 transition-all bg-white sticky top-0 z-50">
                <Link href="/store" className="relative text-xl md:text-3xl lg:text-4xl font-semibold text-slate-700 flex-1">
                    {storeName}<span className="text-green-600 text-2xl md:text-4xl lg:text-5xl leading-0">.</span>
                </Link>
                
                <div className="flex items-center gap-1.5 md:gap-2 lg:gap-3">
                    {/* Announcements Badge */}
                    <Link href="/store/announcements" className="hidden md:block">
                    ...
                    <button 
                        ...
                        className={`md:hidden p-1.5 rounded-lg transition-all duration-300 hamburger-icon ${menuOpen && !menuClosing ? 'bg-green-600 text-white shadow-lg scale-105' : 'bg-slate-100 text-slate-700 hover:bg-slate-200 active:scale-95'}`}
                    >
                        ...
                        {storeLinks.map((link, index) => (
                            <Link
                                ...
                                className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-300 ${
                                    pathname === link.href
                                        ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md font-medium'
